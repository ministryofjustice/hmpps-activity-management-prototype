    ...
    if (prisoner.attendance) {
      ...
      if (
        ...
        record = records.reduce((mostRecentRecord, currentRecord) => {
          return new Date(currentRecord.timestamp.date + " " + currentRecord.timestamp.time) >
            new Date(mostRecentRecord.timestamp.date + " " + mostRecentRecord.timestamp.time)
            ? currentRecord
            : mostRecentRecord;
        });
      } else {
...
router.post("/activities/:selectedDate/:selectedPeriod/:activityId/confirm-cancellation", function (req, res) {
  ...
  if (req.session.data["confirm-cancellation"] == "yes") {
    ...
    let newActivities = req.session.data["timetable-complete-1"]["activities"].filter(
...
router.get(
  ...
  function (req, res) {
    ...
    res.render(req.protoUrl + "/check-print-incentive-level-warning", {
...
router.get(
  ...
  function (req, res) {
    res.render(req.protoUrl + "/confirm-print-incentive-level-warning");
  }
);
    ...
    if (attendanceData && attendanceData[activity.id]) {
      ...
      prisonersWithEvents.forEach((prisoner) => {
        ...
        if (
          ...
          attendanceData[activity.id][date][period][prisoner.id].length
        ) {
          ...
          let lastRecord =
            attendanceData[activity.id][date][period][prisoner.id][
              attendanceData[activity.id][date][period][prisoner.id].length - 1
            ];
...
router.get("/activities/:selectedDate", function (req, res) {
  ...
  if (req.query.search) {
    ...
    activitiesForDate.afternoon = activitiesForDate.afternoon.filter((activity) =>
      activity.name.toLowerCase().replace(/\s/g, "").includes(searchTerm)
    );
  }
